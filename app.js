...
async function loadDefaultMapping(){
  ...
  const r=await fetch('mapping-produit.json?v=2.0.0',{cache:'no-store'});
...
function renderMapping(){
  ...
  $("mappingSources").innerHTML=sources.map(({cfg})=>`<div class="mapping-node source-node ${cfg.target_column?'mapped':'unmapped'} ${mappingSelectedSource===cfg.json_field?'selected':''}" data-source="${mappingEsc(cfg.json_field)}" tabindex="0"><div class="mapping-port source-port" data-port-source="${mappingEsc(cfg.json_field)}" title="Relier ce champ">●</div><div class="mapping-node-body"><strong>${mappingEsc(cfg.json_field)}</strong><small>${mappingEsc(cfg.identify||'Information à identifier dans le document')}</small></div>${mappingBadge(cfg)}</div>`).join('');
  const targets=mappingTargetMeta();
  $("mappingTargets").innerHTML=[...targets.entries()].sort((a,b)=>a[0].localeCompare(b[0],'fr')).map(([name,meta])=>`<div class="mapping-node target-node" data-target="${mappingEsc(name)}"><div class="mapping-port target-port" data-port-target="${mappingEsc(name)}" title="Cible Grist">●</div><div class="mapping-node-body"><strong>${mappingEsc(name)}</strong><small>${mappingEsc(meta.type||'Colonne cible')}</small></div>${String(meta.type||'').startsWith('Ref:')?'<span class="mapping-badge ref">Ref</span>':''}</div>`).join('');
...
function bindMappingNodes(){
  document.querySelectorAll('[data-source]').forEach(n=>{
    n.onclick=e=>{if(e.target.closest('.source-port'))return;selectMappingSource(n.dataset.source)};
    n.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectMappingSource(n.dataset.source)}};
  });
  document.querySelectorAll('[data-port-source]').forEach(p=>{
    p.onpointerdown=e=>{e.preventDefault();e.stopPropagation();mappingPendingSource=p.dataset.portSource;selectMappingSource(mappingPendingSource);document.body.classList.add('mapping-connecting')};
  ...
  document.querySelectorAll('[data-port-target]').forEach(p=>{
    p.onpointerup=e=>{e.preventDefault();e.stopPropagation();if(mappingPendingSource)connectMapping(mappingPendingSource,p.dataset.portTarget)};
    p.onclick=e=>{e.stopPropagation();if(mappingPendingSource)connectMapping(mappingPendingSource,p.dataset.portTarget)};
  });
}
window.addEventListener('pointerup',()=>{document.body.classList.remove('mapping-connecting')});
...
function ensureMappedEntry(jsonField,target){
  ...
  if(targetInfo?.type&&!e.cfg.target_type)e.cfg.target_type=targetInfo.type;
  return e;
}
...
function drawMappingLines(){
  ...
  mappingAllEntries().filter(e=>e.cfg.target_column).forEach(({cfg})=>{
    ...
    const p=document.createElementNS('http://www.w3.org/2000/svg','path');p.setAttribute('d',`M ${x1} ${y1} C ${x1+dx} ${y1}, ${x2-dx} ${y2}, ${x2} ${y2}`);p.setAttribute('class','mapping-link'+(mappingSelectedSource===cfg.json_field?' selected':''));p.dataset.source=cfg.json_field;p.style.pointerEvents='stroke';p.onclick=()=>selectMappingSource(cfg.json_field);svg.appendChild(p);
  });
}
...
function renderMappingRuleEditor(jsonField){
  ...
  $("mappingRuleEditor").innerHTML=`
    ...
    ${ruleField('Colonne Grist',`<input data-map-edit="target_column" value="${mappingEsc(c.target_column||'')}" placeholder="Non mappé">`,'Destination dans Fonctionnalites.')}
